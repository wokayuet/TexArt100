...
#region Bloom
        ...
        void SetupBloom(CommandBuffer cmd, RTHandle source, Material uberMaterial)
        {
            ...
            // Determine the iteration count,估算迭代次数
            int maxSize = Mathf.Max(tw, th);
            int iterations = Mathf.FloorToInt(Mathf.Log(maxSize, 2f) - 1);
            int mipCount = Mathf.Clamp(iterations, 1, m_Bloom.maxIterations.value);

            // Pre-filtering parameters，预滤波参数（阈值/软膝/Clamp/Scatter）
            float clamp = m_Bloom.clamp.value;
            float threshold = Mathf.GammaToLinearSpace(m_Bloom.threshold.value);// threshold 要从 γ 转到 Linear
            ...
            CoreUtils.SetKeyword(bloomMaterial, ShaderKeywordStrings.UseRGBM, m_UseRGBM); // 低精度 RT 时启用 RGBM 编码
            ...
            // Pass 0：Prefilter（筛亮、软膝、Clamp、编码）
            ...
            // Pass 3：向上重建（Upsample + Scatter 混合）
            ...
            // 结果传回 UberPost.shader
            // Setup bloom on uber
            // Tint 归一化到等亮度，避免染色导致变亮/变暗
